 ...
 * <p>
 * The shadowRoot property on the client is represented as a DocumentFragment. Thus, if one tries to
 * return the shadowRoot property from the client via executeScript(), Selenium will simply
 * serialize the object rather than returning a WebElement.
 * <p>
 * Important notes about this class: - Not all WebElement APIs are supported on the shadowRoot
...
public class ShadowRootWebElement implements WebElement, WrapsElement, WrapsDriver {
  ...
  public List<WebElement> findElements(By by) {
    ...
    Object elements = executor.executeScript(
        "return arguments[0].shadowRoot.querySelectorAll('" + escapeForQuery(queryString) + "')",
        rootElement);
    ...
    try {
      ...
      // Add items in the order according to their key. That is the order in which they are found using querySelector.
  ...
  public WebElement findElement(By by) {
    ...
    WebElement ret = (WebElement) executor
        .executeScript(
            "return arguments[0].shadowRoot.querySelector('" + escapeForQuery(queryString) + "')",
            rootElement);
